...
// Capture any contact info that's passed
    ...
    <td>
      <table border="1" cellpadding="1" cellspacing="0" bordercolor="#CCCCCC">
        <tr bordercolor="#CCCCCC" bgcolor="#99CC33">
          <th height="35"><strong><i class="icon-user"></i> Shop User </strong></th>
    		  <th height="35" bgcolor="#99CC33"><strong><i class="icon-list"></i> Status</strong></th>
    		  <th width="70" height="35"><strong><i class="icon-time"></i> Time In </strong></th>
    		  <th width="70" height="35"><strong><i class="icon-time"></i> Time Out </strong></th>
    		  <th height="35"><strong><i class="icon-refresh"></i> Update Hours </strong></th>
    		  <th height="35"><strong>Edit</strong></th>
    		  <th height="35"><strong>Delete Visit</strong></th>
  	    </tr>
        <form method="post" name="form_new" action="<?php echo $editFormAction; ?>">
		  ...
		  <td valign="bottom"><strong>
		    <?php list_contacts_select_role('user_role','Personal'); ?>
		    </strong></td>
		  <td valign="bottom"><strong>
		  ...
		  <td valign="bottom"><input class="btn btn-large btn-primary" name="Submit" type="submit" value="Sign In" /></td>
		  <td valign="bottom">&nbsp;</td>
		  <td valign="bottom">&nbsp;</td>
	    </tr>
	  ...
	  <tr valign="bottom" bordercolor="#CCCCCC" bgcolor="#99CC33">
	    <td class="title todays-users" colspan="7">Today's Shop Users:</td>
	      </tr>
	  ...
	  if($visit_id == $row_Recordset1['shop_visit_id']) {?>
        <form method="post" name="FormUpdate_<?php echo $row_Recordset1['shop_visit_id']; ?>" action="<?php echo $editFormAction; ?>">
          <tr class="visit-detail" valign="bottom">
            <td class="title" colspan="7">Edit Record</td>
          </tr>
          <tr class="visit-detail" valign="bottom">
            <td><?php list_contacts('contact_id', $row_Recordset1['contact_id']); ?></td>
      		  ...
      		  <td class='btn-cell'>
        		  <input type="submit" class="btn btn-primary" name="Submit" value="Update Changes" />
      		  </td>
      		  <td class='btn-cell'><a class="btn btn-disabled btn-small" href=""><i class="icon-pencil"></i> Edit</a></td>
      		  <td class='btn-cell'><a class="btn btn-warning btn-small" href="<?php echo PAGE_SHOP_LOG_DELETE_VISIT . "?visit_id={$visit_id}&shop_id={$shop_id}";?>"><i class="icon-trash"></i> Delete</a></td>
    	    </tr>
	  ...
	  <?php } else { //This section executes if it is not the visit_id selected NOT FOR EDIT ?> 
        <form method="post" name="FormUpdate_<?php echo $row_Recordset1['shop_visit_id']; ?>" action="<?php echo $editFormAction; ?>">
          <tr class="visitor-row" bordercolor="#CCCCCC">
            <td><a href="<?php echo "{$page_individual_history_log}?contact_id=" . $row_Recordset1['contact_id']; ?>"><?php echo ucwords($row_Recordset1['full_name']); ?></a></td>
		  ...
		  <td class='btn-cell'>
		    <a class="btn btn-small" href="<?php echo "{$_SERVER['PHP_SELF']}?shop_id={$shop_id}&visit_id={$row_Recordset1['shop_visit_id']}"; ?>"><i class="icon-pencil"></i> Edit</a>
		  </td>
		  <td class='btn-cell'>
		    <a class="btn btn-small btn-warning" href="<?php echo PAGE_SHOP_LOG_DELETE_VISIT . "?visit_id={$row_Recordset1['shop_visit_id']}&shop_id={$shop_id}";?>"><i class="icon-trash"></i> Delete</a>
		  </td>
	    </tr>
